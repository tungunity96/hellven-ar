    ...
    var MeshBatcher = (function (_super) {
      ...
      function MeshBatcher(maxVertices, materialCustomizer) {
        ...
        if (materialCustomizer === void 0) {
          materialCustomizer = function (parameters) {};
        }
        var _this = _super.call(this) || this;
        ...
        var vertexBuffer = (_this.vertexBuffer = new THREE.InterleavedBuffer(
          vertices,
  ...
  (function (threejs) {
    var ThreeJsTexture = (function (_super) {
      ...
      ThreeJsTexture.toThreeJsTextureFilter = function (filter) {
        if (filter === spine.TextureFilter.Linear) return THREE.LinearFilter;
        else if (filter === spine.TextureFilter.MipMap) return THREE.LinearMipMapLinearFilter;
        else if (filter === spine.TextureFilter.MipMapLinearNearest) return THREE.LinearMipMapNearestFilter;
        else if (filter === spine.TextureFilter.MipMapNearestLinear) return THREE.NearestMipMapLinearFilter;
        else if (filter === spine.TextureFilter.MipMapNearestNearest) return THREE.NearestMipMapNearestFilter;
        else if (filter === spine.TextureFilter.Nearest) return THREE.NearestFilter;
        else throw new Error("Unknown texture filter: " + filter);
      };
      ThreeJsTexture.toThreeJsTextureWrap = function (wrap) {
        if (wrap === spine.TextureWrap.ClampToEdge) return THREE.ClampToEdgeWrapping;
        else if (wrap === spine.TextureWrap.MirroredRepeat) return THREE.MirroredRepeatWrapping;
        else if (wrap === spine.TextureWrap.Repeat) return THREE.RepeatWrapping;
